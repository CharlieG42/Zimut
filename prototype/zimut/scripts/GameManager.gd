...
func advance_level() -> void:
	current_level += 1
	# Réinitialiser la grille et les entités
	init_grid()
	init_entities()
	_init_rocks()
	...
	current_turn = 0
	turn_count = 1
	# Réinitialiser les compteurs de sorts spéciaux
	destruction_spells_used = 0
	teleport_spells_used = 0
	...
	if players.size() > 0:
		...
		_reset_special_spells()
	turn_changed.emit(current_turn)
	message_requested.emit("Niveau %d - Nouvelle carte générée !" % current_level)
	_refresh_grid()
...
func check_game_over() -> void:
	...
	if alive_p == 0:
		...
		# Au lieu de finir le jeu, passer au niveau suivant
		advance_level()
