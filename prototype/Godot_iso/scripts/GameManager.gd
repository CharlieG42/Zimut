...
# Nouvelle variable pour stocker l'Ã©quipe personnalisÃ©e
...
func _get_spell_damage(spell: Dictionary, damage_type: String) -> int:
	...
	if damage_type == "physical" and spell.has("CoÃ»t PA"):
		pass  # Not applicable
...
## Nouvelle mÃ©thode pour rÃ©initialiser l'Ã©quipe personnalisÃ©e
...
func _init_with_fallback_data() -> void:
	...
	add_child(dl_node)
	# Forcer le chargement maintenant que le nÅud existe
	_on_data_loaded()
...
func init_entities() -> void:	# Utiliser l'Ã©quipe personnalisÃ©e si elle est dÃ©finie, sinon utiliser les classes par dÃ©faut
	var player_classes: Array = []
	var player_positions: Array[Vector2i] = [Vector2i(1, 1), Vector2i(2, 1), Vector2i(1, 2)]
	
	if custom_team.size() == 3:
		# Utiliser l'Ã©quipe personnalisÃ©e
	...
	for i: int in range(player_classes.size()):
		...
		# Si on utilise l'Ã©quipe personnalisÃ©e, on utilise les stats fournies
				...
				if spell_info.get("Classe", "") == classe:
					...
					if req_lvl <= DEFAULT_PLAYER_LEVEL:
						player["spells"].append({
							...
							"classe":            classe,
							"cost_pa":           int(spell_info.get("Cout_PA", spell_info.get("CoÃ»t PA", "1"))),
							"cost_pm":           int(spell_info.get("Cout_PM", spell_info.get("CoÃ»t PM", "0"))),
							"range":             int(spell_info.get("Portee", spell_info.get("PortÃ©e", "1"))),
		...
		var player: Dictionary = {
			...
			"agility":      _csv_int(class_info, "AgilitÃ© (Vit. Atk)",    10),
			"wisdom":       _csv_int(class_info, "Sagesse (PrÃ©cision)",   10),
			"defense":      _csv_int(class_info, "DÃ©fense",               10),
	...
	for i: int in range(enemy_types.size()):
		...
		var enemy: Dictionary = {
			...
			"wisdom":       0,
			"defense":      _csv_int(enemy_info, "DÃ©fense",   5),
...
func _try_cast_spell(caster: Dictionary, target_pos: Vector2i, target_entity) -> void:
	...
	elif spell_type == "Buff" or spell_type == "DÃ©fense":
		actual_target = caster
	else:
