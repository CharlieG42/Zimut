...
## Correction : pas de disconnect() sur signaux non connectés
...
func _connect_signals() -> void:
	...
	## (les managers s'initialisent une seule fois)
	...
	# GameManager → UIManager  (connectés dans UIManager.init via _setup_connections)
	# (déjà gérés dans UIManager._setup_connections, pas besoin de redoubler ici)
