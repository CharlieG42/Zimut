...
func _process_single_enemy(enemy):
	# Trouver le joueur vivant le plus proche
	var closest_player = _find_closest_alive_player(enemy)
	if closest_player.is_empty():
		return

	# Si déjà adjacent, attaquer directement
	var dist_to_player = abs(int(enemy["x"]) - int(closest_player["x"])) + abs(int(enemy["y"]) - int(closest_player["y"]))
	...
	var new_dist = abs(int(enemy["x"]) - int(closest_player["x"])) + abs(int(enemy["y"]) - int(closest_player["y"]))
...
func _find_closest_alive_player(enemy):
	var closest_player = {}
	var min_dist = 9999
	for player in game_manager.players:
		if int(player["current_pv"]) > 0:
			var d = abs(int(enemy["x"]) - int(player["x"])) + abs(int(enemy["y"]) - int(player["y"]))
			if d < min_dist:
				min_dist = d
...
func _move_towards(enemy, target):
	var ex = int(enemy["x"])
	var ey = int(enemy["y"])
	var px = int(target["x"])
	var py = int(target["y"])
...
func _attack_enemy(attacker, target):
