...
var _cached_sprite_path: String = ""
...
func set_in_spell_range(value: bool) -> void:
...
## Essayer de charger le sprite pour une entité (avec cache)
func _try_load_sprite(classe: String, entity_type: String) -> bool:
	...
	# Éviter de recharger si déjà le bon sprite en cache
	if sprite_path == _cached_sprite_path and entity_sprite.texture != null:
		return true

	if ResourceLoader.exists(sprite_path):
		var tex: Resource = load(sprite_path)
		if tex is Texture2D:
			entity_sprite.texture = tex as Texture2D
			# Sprite flottant centré : 56px de large, aligné bas sur le centre de la tuile
			var tex_size: Vector2 = entity_sprite.texture.get_size()
			var target_w: float = 56.0
			var scale_f: float = target_w / maxf(tex_size.x, 1.0)
			entity_sprite.scale = Vector2(scale_f, scale_f)
			# Décaler vers le haut pour que le bas du sprite soit au centre de la tuile
...
func _draw_health_bar(center: Vector2) -> void:
	...
	if ratio < 0.5:  fill_color = Color(0.95, 0.75, 0.1)
	if ratio < 0.25: fill_color = Color(0.95, 0.15, 0.1)
...
func _draw_ellipse(center: Vector2, radius: Vector2, color: Color, steps: int = 20) -> void:
	var pts  := PackedVector2Array()
	var cols := PackedColorArray()
...
func _colors3(c: Color) -> PackedColorArray:
	return PackedColorArray([c, c, c])

func _colors4(c: Color) -> PackedColorArray:
...
func _is_point_in_diamond(point: Vector2) -> bool:
	## Losange défini par (0,50)-(50,0)-(100,50)-(50,100)
	## Formule : |px - cx| / (w/2) + |py - cy| / (h/2) <= 1
	var cx := 50.0
	var cy := 50.0
	var hw := 50.0   # demi-largeur
	var hh := 50.0   # demi-hauteur
