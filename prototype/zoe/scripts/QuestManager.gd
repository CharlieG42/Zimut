...
func start_quest(quest_id: String) -> Quest:
	...
	var quest_data = available_quests[quest_id]
...
func complete_quest(quest_id: String) -> void:
	...
	# Apply rewards
	if player_node:
		var player_data := {"xp": 0, "berries": 0, "water": 0, "hunger": 0, "thirst": 0}
		quest.apply_rewards(player_data)
		
		# Apply rewards to player
		if player_data.has("hunger"):
			player_node.hunger = min(100, player_node.hunger + player_data["hunger"])
		if player_data.has("thirst"):
			player_node.thirst = min(100, player_node.thirst + player_data["thirst"])
