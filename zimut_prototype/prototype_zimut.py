#!/usr/bin/env python3
"""
Prototype Zimut - Combat tour par tour
"""
...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption("Prototype Zimut - Combat Tour par Tour")
        self.font = pygame.font.SysFont('Arial', 16)
        self.font_large = pygame.font.SysFont('Arial', 24)
        self.font_title = pygame.font.SysFont('Arial', 32, bold=True)
        self.clock = pygame.time.Clock()

        self.classes_data = self.load_csv(CLASSES_FILE)
        self.spells_data = self.load_csv(SORTS_FILE)
        self.enemies_data = self.load_csv(ENNEMIS_FILE)
        self.stuff_data = self.load_csv(STUFF_FILE)
    ...
    def init_entities(self):
        for i, (classe, level, pos) in enumerate(zip(PLAYER_CLASSES, PLAYER_LEVELS, PLAYER_POSITIONS)):
            class_data = [d for d in self.classes_data if d["Classe"] == classe and d["Niveau"] == str(level)]
            if class_data:
                data = class_data[0]
                player = Player(
                    name=f"{classe} Lv{level}",
                    entity_type="Player",
                    classe=classe,
                    level=level,
                ...
                for spell_data in self.spells_data:
                    if spell_data["Classe"] == classe and int(spell_data["Niveau requis"]) <= level:
    ...
    def handle_spell_selection(self, spell_index: int):
        ...
        if spell_index < len(current_player.spells):
            spell = current_player.spells[spell_index]
            if spell.can_cast(current_player):
                self.selected_spell = spell
            else:
                print(f"Pas assez de PA/PM pour {spell.name}")
    ...
    def draw_ui(self):
        ...
        if self.current_turn == 0:
            current_player = self.players[self.current_player_index]
            player_info = f"Joueur: {current_player.name} | PV: {current_player.current_pv}/{current_player.max_pv} | PA: {current_player.current_pa}/{current_player.max_pa} | PM: {current_player.current_pm}/{current_player.max_pm}"
            player_surface = self.font.render(player_info, True, BLACK)
            self.screen.blit(player_surface, (MARGIN, SCREEN_HEIGHT - 80))

            if not self.show_spells:
                instructions = "Sélectionnez votre personnage pour voir ses actions."
            else:
                instructions = "Choisissez une action : Déplacement (clic), Attaque (clic), Sort (1-5 + clic)."
    ...
    def draw_spells(self):
        ...
        spell_panel = pygame.Rect(MARGIN, SCREEN_HEIGHT - 150, 300, 140)
        pygame.draw.rect(self.screen, (255, 255, 255, 200), spell_panel)
        ...
        for i, spell in enumerate(current_player.spells[:5]):
            can_cast = spell.can_cast(current_player)
            color = BLACK if can_cast else DARK_GRAY
            spell_text = self.font.render(f"{i+1}. {spell.name} (PA:{spell.cost_pa}, PM:{spell.cost_pm})", True, color)
